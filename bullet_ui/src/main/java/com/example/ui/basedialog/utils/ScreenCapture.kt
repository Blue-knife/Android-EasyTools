 ...
 */
val Context.staticBarHeight: Int
    get() {
        var result = 0
...
val Context.isNetWorkAvailable: Boolean
    ...
    get() {
        if (checkCallingOrSelfPermission(Manifest.permission.INTERNET)
 ...
 */
val Context.checkDeviceHasNavigationBar: Boolean
...
@RequiresApi(Build.VERSION_CODES.LOLLIPOP)
fun Context.getNavigationBarHeight(activity: Activity): Int {
 ...
 * 获取手机内的分享入口
 */
fun Context.queryShareItems(): MutableList<ResolveInfo> {
    ...
    //查询可以使用文本分享的所有入口
    val resolve = packageManager.queryIntentActivities(intent, 0)
    resolve.forEach {
        shareList.add(it)
        //只保留 qq 和 微信的入口
